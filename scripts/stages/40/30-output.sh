...
color_echo "size of boot partition: $SIZE_BOOT_FS ($(du -sh "$BOOTFS_PATH" | sed "s/\s.*//") used)" -Yellow
color_echo "size of root partition: $SIZE_ROOT_FS ($(du -sh "$ROOTFS_PATH" | sed "s/\s.*//") used)" -Yellow
color_echo "size of data partition: $SIZE_DATA_FS ($(du -sh "$DATAFS_PATH" | sed "s/\s.*//") used)" -Yellow
