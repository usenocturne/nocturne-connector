...
color_echo "size of sdcard image: $(du -sh "$OUTPUT_PATH"/nocturne-connector_"$XBPS_ARCH".img.gz | sed "s/\s.*//")" -Yellow
color_echo "size of update: $(du -sh "$OUTPUT_PATH"/nocturne-connector_update_"$XBPS_ARCH".img.gz | sed "s/\s.*//")\n" -Yellow
