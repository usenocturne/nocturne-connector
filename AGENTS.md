...
Bluetooth device `connected` state in the shared UI comes from active RFCOMM routes, not the lower-level Windows ACL status. Resolve active connection labels through the normalized device cache and display generated `Bluetooth <address>` names as Unknown Device. Discovery is user-initiated only and has a service-owned 30-second deadline. Manual Disconnect cancels reconnect state and closes every matching inbound and outbound route without removing the bond. Unpair suppresses late ACL and in-flight connection callbacks, closes every route first, then removes every paired WinRT identity and the classic bond; custom pairing must select a classic identity and fall back to the BR/EDR API when Windows exposes only an LE watcher identity.
