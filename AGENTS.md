...
- **Metadata-only lyrics use LRCLIB directly.** Accept canonical and legacy content-id, title, and artist aliases at the RPC boundary. Requests without a Spotify content id and `spotify:local:` items skip Spotify color lyrics. Every LRCLIB URL contains only `track_name` and `artist_name`, even when a legacy caller supplies album or duration metadata, and uses the conventional unbranded browser user agent from the Spotify web configuration.
- **Spotify local-file library writes use collection v2.** Route `spotify:local:` save and remove requests through the regional spclient `/collection/v2/write` protobuf endpoint with the Spotify username, `collection` set, epoch-second `added_at`, and a fresh 16-hex-character `client_update_id`; removals additionally set `is_removed`. Keep ordinary track mutations and all contains checks on Pathfinder. Do not invent collection-v2 page or contains behavior without a verified wire capture.
...
- **Replacement RPC ownership:** A new connection for an existing route must clean up the previous RPC client even when no disconnect event preceded it. Reject its pending calls and prevent stale writers or delayed handshakes from using the replacement transport. Prepared OTA recovery must rebind through the current RPC client.
- **Device info is normalized at the RPC boundary.** The daemon's canonical `device.info` response uses snake-case wire fields, while the Connector UI and analytics consume camel-case properties. Normalize canonical and legacy spellings before caching, broadcasting, or serving device info so all consumers see the same typed shape.
...
- **Connector auth is fail-stable.** Temporary auth API responses, network failures, and UI status request failures must never route an authenticated user back to pairing. Supabase refresh retries include HTTP 408, 425, 429, and every 5xx response. Only explicit sign-out or a definitively rejected session clears local identity. Persist rotated session tokens atomically with mode `0600` on POSIX and Windows DPAPI protection on Windows. Serialize protected writes, keep the newest in-memory rotation during runtime recovery, retry unavailable protected-session reads, and drain pending writes before sign-out removes the saved session. Account pairing must receive an independent session from the website, never a copy of its browser refresh-token family.
- **A/B boot contract:** BOOT is mounted at `/uboot`, root A is partition 2, root B is partition 3, data is partition 4. U-Boot stores slot state in `/uboot/uboot.dat`; the connector helper writes the CRC in U-Boot big-endian order while accepting legacy little-endian records during migration. `uboot-boot-success` resets the boot counter after local mounts. Do not remove that service or healthy devices can roll back after repeated boots.
...
- **Encryption helpers live in one place** (`server/utils/encryption.ts`); don't roll your own. Shared Spotify credential reads accept Apple uppercase UUID keys and legacy raw and lowercase UUID keys. Writes retain the original user ID key derivation so released connectors can still read refreshed credentials; changing the shared write format requires a coordinated reader rollout. Temporary Spotify refresh failures retain restored linked state and retry with capped backoff; only a confirmed OAuth rejection after reloading shared credentials may clear authorization.
...
The Windows companion lives under `windows/` plus the platform seams in `src/server/`. It uses a Tauri tray host with a lazily created WebView2 window and a compiled Bun server sidecar. The shared client UI is never forked.

```bash
just windows-check
just windows-host-test
just windows-server
just windows-test
just windows-universal
```

Connector releases use an independent `2.x` version stream. A version bump updates
`build.sh`, `src/package.json`, `windows/src-tauri/Cargo.toml`,
`windows/src-tauri/tauri.conf.json`, `windows/scripts/build-server.ts`, and
`windows/scripts/build-universal.ps1`; run Cargo afterward so `windows/Cargo.lock`
records the new host package version. Keep the Pi image, Windows host, Bun sidecar,
and installer versions aligned.

The host binds RFCOMM channel 1 for Pi-compatible SPP when available (falling back to an automatically allocated channel while advertising the same SPP UUID when Windows reserves channel 1), attempts channel 3 for the desktop probe, and uses channel 2 for the outbound Car Thing route. Windows commonly reserves channel 3 for its built-in incoming serial service, so the probe listener is best effort and paired `Nocturne` devices must also recover through a direct channel 2 dial on startup, adapter return, and route loss. Its owner-ACL named-pipe bridge uses a four-byte little-endian length followed by MessagePack, a per-process token, and connection generations. Windows media events preserve the macOS `media_generation`, metadata-before-artwork ordering, and 300 px JPEG artwork limit. `app.ready.platform` remains `web` for released firmware compatibility.

Windows-specific behavior must never change the Raspberry Pi render or runtime defaults. The native host marks its loopback WebView URL with `connector-platform=windows`; the shared UI requires that marker, a Windows browser user agent, and a loopback host before enabling Windows presentation. A Pi opened from a Windows browser remains on Linux behavior, including through a loopback tunnel unless the native marker is explicitly present. Pi retains its original header spacing, mandatory Spotify setup, 60-second client-owned Bluetooth scan, BlueZ ACL connection badges, lack of a manual Disconnect control, original connection-list shape and `Device` fallback, original unpair and auto-connect policy, standard keepalive timeout, disk static assets, `0.0.0.0:80` bind defaults, and legacy OTA response framing. Add explicit Linux parity tests for every shared platform seam.

Bluetooth device `connected` state in the shared UI comes from active RFCOMM routes, not the lower-level Windows ACL status. Resolve active connection labels through the normalized device cache and display generated `Bluetooth <address>` names as Unknown Device. Discovery is user-initiated only and has matching sidecar and native 30-second deadlines, and native generations prevent a stale scan worker from extending or stopping its replacement. Windows never changes OS discoverability during startup or adapter hotplug, remains connectable for its RFCOMM listeners, and sidecar initialization cancels any orphaned native discovery left by a prior sidecar. Stop discovery before starting Windows authentication. Windows custom pairing advertises only `ConfirmPinMatch`. Show the same six-digit code as the Car Thing and require explicit confirmation or rejection on Windows. Never request PIN entry, supply a fixed PIN, autoaccept a code-free ceremony, or fall back to the Win32 wizard. Require encryption and authentication in WinRT; unsupported devices need a current Car Thing image and Bluetooth driver. Native deferrals wait off the callback thread, expire after 60 seconds, and scope responses and cancellation to a unique requestId. Windows confirmation routes await native results and preserve the pending request on validation or bridge failure; the shared UI restores it on remount and exposes errors. Pi pairing defaults stay unchanged. Log the selected endpoint, ceremony (without logging the code), named numeric result, and elapsed pairing duration. Present `AuthenticationTimeout` with guidance to open the Car Thing pairing screen, keep it nearby, complete any Windows prompt, and retry; a timeout alone does not identify the underlying cause. Propagate native errors to the UI. Manual Disconnect cancels reconnect state and closes every matching inbound and outbound route without removing the bond. Windows Unpair closes every route and removes only the local Windows identities and classic bond; it must never ask the Car Thing to remove its BlueZ bond. Unpair suppresses late ACL and in-flight connection callbacks throughout that sequence. Custom pairing must resolve a fresh classic identity from the Bluetooth address so immediate repair never depends on another scan, and report an actionable error when Windows exposes no usable WinRT classic identity.

Windows device-list responses stably partition recognized `Nocturne` names ahead of all other discoveries as soon as refresh data arrives. Preserve discovery order within both groups and return the adapter's original order unchanged on Pi.

The host owns exactly one runtime tray icon; `tauri.conf.json` must not declare another one. Login startup and installer restart pass `--background`, create no window or WebView, and remain in the notification area. A normal cold launch requests one UI window and waits for sidecar readiness; any later invocation focuses that same UI. The Open action creates the shared UI lazily and closing it destroys the WebView while leaving the host and Bun sidecar alive. Sidecar exit clears readiness, supersedes the old named-pipe connection, and clears native RFCOMM routes before the replacement reconnects. Never call a helper that locks the Bluetooth state from inside an existing Bluetooth-state lock.

Windows system media matches the macOS companion setting: it defaults on, persists to `system-media-enabled.json`, and appears as the System media switch in the Windows Settings Media section. Disabling emits a final stopped update, clears metadata and artwork caches, stops volume reporting, and makes controls return `disabled`; enabling restarts monitoring and replay. Spotify-skipped mode forces media active and disables the switch. Track changes and player replacement use both source ID and COM session identity so a restarted player cannot retain a stale control target or artwork epoch. GSMTC timeline positions must be projected from `LastUpdatedTime` to emission time and sent as canonical elapsed milliseconds plus a positive playback-rate multiplier. Keep the 250 ms state poll, reanchor playing progress at most once per second unless status, rate, duration, or a seek changes, and rebase cached playing anchors before Car Thing replay so reconnection cannot move progress backward.

Spotify skip matches the macOS companion. Persist it in `spotify-skipped.json`, expose it through the Spotify API and setup UI, retain it during a relink attempt, and clear it only after authorization succeeds. While the persisted flag is set, `spotify.auth.status`, `spotify.auth.get_status`, and `app.ready` report skipped and system media remains forced on even while authorization is loading or polling.

Windows state belongs under `%LOCALAPPDATA%\\Nocturne\\Connector`; the per-user installer registers login startup and never removes that state on uninstall. Connector self-updates, host reboot, LAN serving, USB-NCM networking, and WinNAT are intentionally unsupported. Car Thing OTA remains enabled.
